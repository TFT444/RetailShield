...
const ATTACK_SIM_EVENTS = [
  {
    ...
    playbook: {
      trigger: 'quarantine_email',
      auto: ['Email quarantined across all mailboxes via Defender for O365','Sender domain dr4gonm4il.com added to tenant block list','SHA256 hash pushed to RetailIOCWatchlist','CFO and EA notified via Teams','Incident ticket raised: #SIM-001'],
      manual: ['Confirm CFO did not open attachment before quarantine','Check for related campaign in last 72h mail gateway logs','Review sender domain registration — likely <7 days old','Escalate to CISO if attachment was executed','Submit to Microsoft MSRC threat intelligence'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'block_ip',
      auto: ['89 source IPs added to Azure WAF deny rules','CAPTCHA enforcement enabled on /pos-admin/login','4 compromised accounts locked and password reset triggered','Rate limiting set to 3 attempts / 5 min per account','Indicators submitted to AbuseIPDB'],
      manual: ['Force reset all 31 targeted accounts preventatively','Check if compromised accounts accessed transaction data','Assess PCI DSS notification requirement','Tune WAF rules for similar User-Agent rotation patterns','Monitor for continued campaign over next 48h'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'isolate_endpoint',
      auto: ['WORKSTATION-SIM-07 isolated via Defender for Endpoint API','Network access revoked at Azure Firewall','C2 IP 91.234.55.12 added to block list across all firewalls','Forensic memory snapshot and disk image initiated','P1 incident raised — on-call SOC paged via PagerDuty'],
      manual: ['Confirm no lateral movement to adjacent workstations','Scope shadow copy destruction across all network shares','Engage IR retainer if spread confirmed to >3 hosts','Start GDPR Art.33 72h notification clock','Prepare operational comms for store management'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'data_exfil_contain',
      auto: ['DNS to d4t4pipe[.]xyz blocked at Sentinel-managed resolver','Domain added to RetailIOCWatchlist and sinkholes','db_svc_account password reset and sessions revoked','Network flow evidence preserved in Log Analytics','DLP alert raised for \\customers\\ folder bulk read'],
      manual: ['Determine initial access vector to DB-SERVER','Assess what customer PII was staged — GDPR impact assessment','Engage Data Protection Officer immediately','Determine if exfiltration completed before DNS block','Audit db_svc_account activity for prior 72h'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'notify_soc',
      auto: ['Call recording preserved under legal hold','Spoofed number added to VOIP block list','Finance team fraud awareness alert issued','Report filed with Action Fraud (ref: SIM-NCSC-2026)'],
      manual: ['Verify with Regional MD via known direct mobile','Issue mandatory voice fraud refresher to finance team','Confirm dual-approval protocol was followed correctly','Submit deepfake audio to NCSC AI threat analysis team','Update payment policy: add video call re-confirmation step'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'notify_soc',
      auto: ['svc_backup_sim flagged for elevated monitoring','Risky sign-in logged in Azure AD Identity Protection','SOC Teams channel alerted with full session context','Session commands captured to forensic audit trail'],
      manual: ['Verify whether svc_backup_sim ever logs in interactively','Cross-reference IP against Tor exit node list','Review all AD changes made during this session','Disable account immediately if compromise confirmed','Audit all accounts from same /24 IP range'],
    },
  },
  {
    ...
    playbook: {
      trigger: 'suspend_terminal',
      auto: ['POS-SIM-TILL-09 suspended via retail management API','Store manager notified via Teams','Memory dump initiated for DLL forensic analysis','ServiceNow ticket raised with PCI DSS flag'],
      manual: ['Physically inspect TILL-09 for hardware skimmer','Pull network capture from last 4 hours on switch port','Identify origin of xf99ab.dll via software deployment logs','Assess payment card data exposure window for PCI notification','Re-image terminal before returning to service'],
    },
  },
]
...
const VULN_INITIAL = {
  ...
  findings: [
    { id:'POS-TILL-01', product:'Oracle Xstore POS', version:'8.1', location:'Hounslow Branch', cat:'POS System', vulns:[
    ...
    { id:'POS-TILL-03', product:'NCR Aloha POS', version:'12.3', location:'Hammersmith Branch', cat:'POS System', vulns:[
    ...
    { id:'POS-TILL-05', product:'Toshiba TCx POS', version:'5.2', location:'Ealing Branch', cat:'POS System', vulns:[
    ...
    { id:'POS-TILL-06', product:'Verifone POS', version:'3.4', location:'Ealing Branch', cat:'POS System', vulns:[
    ...
    { id:'ERP-DYN-01', product:'Microsoft Dynamics Retail', version:'10.0.28', location:'Head Office', cat:'Stock Management', vulns:[
    ...
    { id:'ERP-ORA-01', product:'Oracle Retail Merchandising', version:'21.0', location:'Head Office', cat:'Stock Management', vulns:[
    ...
    { id:'ERP-SAP-01', product:'SAP Retail', version:'S/4HANA 2021', location:'Head Office', cat:'Stock Management', vulns:[
    ...
    { id:'ERP-JDA-01', product:'JDA Supply Chain', version:'9.2', location:'Head Office', cat:'Stock Management', vulns:[
    ...
    { id:'TERM-VFN-01', product:'Verifone VX520', version:'2.1.0', location:'Hounslow Branch', cat:'Payment Terminal', vulns:[
    ...
    { id:'TERM-PAX-01', product:'PAX S920', version:'1.2', location:'Hounslow Branch', cat:'Payment Terminal', vulns:[
    ...
    { id:'TERM-ING-01', product:'Ingenico iCT250', version:'6.0.0', location:'Ealing Branch', cat:'Payment Terminal', vulns:[
    ...
    { id:'TERM-VFN-02', product:'Verifone P400', version:'3.0.1', location:'Hammersmith Branch', cat:'Payment Terminal', vulns:[
    ...
    { id:'PLAT-SHP-01', product:'Shopify POS', version:'9.1.0', location:'Online', cat:'Retail Platform', vulns:[
    ...
    { id:'PLAT-SQR-01', product:'Square POS', version:'5.28', location:'Online', cat:'Retail Platform', vulns:[
    ...
    { id:'PLAT-LSP-01', product:'Lightspeed Retail', version:'2024.1', location:'Online', cat:'Retail Platform', vulns:[
    ...
    { id:'PLAT-RVL-01', product:'Revel POS', version:'4.7', location:'Online', cat:'Retail Platform', vulns:[
...
function SecurityGauge({ score }) {
  ...
  return (
    <svg viewBox="0 0 200 148" style={{ width: '100%', maxWidth: 200 }}>
      ...
      <path d={arc(startDeg, valueDeg)} fill="none" stroke={col} strokeWidth={14} strokeLinecap="round" style={{ filter: `drop-shadow(0 0 6px ${col})` }} />
      <text x={cx} y={cy + 8} textAnchor="middle" fill={col} fontSize={34} fontWeight="bold" fontFamily="monospace">{score}</text>
      <text x={cx} y={cy + 26} textAnchor="middle" fill={C.muted} fontSize={10} fontFamily="sans-serif" letterSpacing="1">SECURITY SCORE</text>
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:3, height:72, marginBottom:6 }}>
        {counts.map((n,i) => (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center' }}>
            <div style={{ width:'100%', height:n===0?2:`${(n/max)*68}px`, background:n>=4?C.red:n>=3?C.orange:n>=1?C.blue:C.border, borderRadius:'2px 2px 0 0', transition:'height 0.5s ease, background 0.3s', boxShadow:n>=3?`0 0 6px ${n>=4?C.red:C.orange}`:'none' }} />
          </div>
        ))}
      </div>
      <div style={{ display:'flex', gap:3 }}>
        {labels.map((l,i) => <div key={i} style={{ flex:1, textAlign:'center', fontSize:8, color:C.dim }}>{l}</div>)}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ marginBottom:12 }}>
      ...
      <div style={{ height:6, background:C.border, borderRadius:3 }}>
        <div style={{ height:'100%', width:`${pct}%`, borderRadius:3, background:color, boxShadow:`0 0 6px ${color}40`, transition:'width 1.2s ease' }} />
      </div>
    </div>
  )
}
...
function PlaybookModal({ threat, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:28, maxWidth:660, width:'100%', maxHeight:'88vh', overflowY:'auto' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
          <div>
            <div style={{ display:'flex', gap:8, marginBottom:8 }}>
              ...
              <span style={{ color:STA[threat.status], padding:'3px 10px', borderRadius:4, fontSize:11, border:`1px solid ${STA[threat.status]}`, background:`${STA[threat.status]}15` }}>{threat.status.toUpperCase()}</span>
              <span style={{ color:SEV[threat.severity], padding:'3px 10px', borderRadius:4, fontSize:11, background:`${SEV[threat.severity]}15` }}>{threat.severity.toUpperCase()}</span>
            </div>
          ...
          <button onClick={onClose} style={{ background:'transparent', border:`1px solid ${C.border}`, color:C.muted, borderRadius:6, padding:'5px 12px', cursor:'pointer', fontSize:18, lineHeight:1 }}>✕</button>
        </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:22 }}>
          ...
          <code style={{ background:'#1a0505', color:C.red, padding:'4px 12px', borderRadius:5, fontSize:12, fontFamily:'monospace', border:`1px solid ${C.redDim}` }}>{threat.playbook.trigger}</code>
        </div>
        <div style={{ marginBottom:22 }}>
          ...
          {threat.playbook.auto.map((step,i) => (
            <div key={i} style={{ display:'flex', gap:10, marginBottom:10, alignItems:'flex-start' }}>
              <div style={{ minWidth:22, height:22, borderRadius:'50%', background:C.green, color:'#000', fontSize:11, fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', marginTop:1, flexShrink:0 }}>✓</div>
        ...
        <div>
          ...
          {threat.playbook.manual.map((step,i) => (
            <div key={i} style={{ display:'flex', gap:10, marginBottom:10, alignItems:'flex-start' }}>
              <div style={{ minWidth:22, height:22, borderRadius:'50%', background:'transparent', color:C.orange, fontSize:11, fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', marginTop:1, flexShrink:0, border:`1.5px solid ${C.orange}` }}>{i+1}</div>
...
export default function RetailShield() {
  ...
  const runVulnScan = useCallback(() => {
    ...
    setVulnScan('scanning'); setVulnProg(0)
    let p = 0
    const step = setInterval(() => {
      p += Math.random() * 14 + 4
      if (p >= 100) {
        clearInterval(step); setVulnProg(100)
        setTimeout(() => { setVulnData({ ...VULN_INITIAL, lastScan: new Date().toISOString() }); setVulnScan('done'); setTimeout(() => setVulnScan(null), 4000) }, 400)
      } else { setVulnProg(Math.round(p)) }
    }, 200)
  }, [vulnScan])
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color:C.text }}>
      ...
      {simStatus && (
        <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:9997, background:simStatus==='running'?'repeating-linear-gradient(90deg,#450a0a 0px,#7f1d1d 40px,#450a0a 80px)':`linear-gradient(135deg, #052e16, #166534)`, borderBottom:`2px solid ${simStatus==='running'?C.red:C.green}`, padding:'9px 24px', display:'flex', alignItems:'center', gap:12, boxShadow:`0 2px 24px ${simStatus==='running'?'rgba(220,38,38,0.7)':'rgba(22,163,74,0.5)'}`, animation:simStatus==='running'?'simPulse 0.9s ease-in-out infinite':'slideIn 0.3s ease' }}>
          <span style={{ fontSize:16 }}>{simStatus==='running'?'⚡':'✅'}</span>
          <span style={{ fontWeight:800, fontSize:12, textTransform:'uppercase', letterSpacing:'1px', color:simStatus==='running'?C.red:C.green }}>{simStatus==='running'?'ATTACK SIMULATION RUNNING':'SIMULATION COMPLETE — 7 threats injected'}</span>
          {simStatus==='running' && <span style={{ fontSize:11, color:C.muted }}>— injecting threats into live feed...</span>}
          <div style={{ marginLeft:'auto', display:'flex', gap:6 }}>
            {simStatus==='running' && ['T1566.001','T1110.004','T1486','T1048','T1598'].map(t => <span key={t} style={{ fontFamily:'monospace', fontSize:10, color:C.red, background:'rgba(0,0,0,0.4)', padding:'2px 6px', borderRadius:3, border:`1px solid ${C.redDim}` }}>{t}</span>)}
          </div>
        </div>
      )}

      {banner && (
        <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:9998, background:`linear-gradient(135deg, ${C.redDim}, #7f1d1d)`, borderBottom:`1px solid ${C.red}`, padding:'10px 24px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 2px 20px rgba(220,38,38,0.5)', animation:'slideIn 0.25s ease' }}>
      ...
      <header style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, padding:'0 24px', height:56, display:'flex', alignItems:'center', position:'sticky', top:banner?42:0, zIndex:200, gap:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginRight:28 }}>
          <div style={{ width:34, height:34, background:C.red, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, boxShadow:`0 0 12px ${C.red}60` }}>🛡</div>
          <div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:16 }}>
          ...
          <button onClick={runAttackSimulation} disabled={simStatus==='running'} style={{ background:simStatus==='running'?'transparent':`linear-gradient(135deg, ${C.red}, #b91c1c)`, color:simStatus==='running'?C.red:'#fff', border:`1.5px solid ${C.red}`, borderRadius:7, padding:'7px 16px', fontSize:11, fontWeight:800, letterSpacing:'0.6px', textTransform:'uppercase', cursor:simStatus==='running'?'not-allowed':'pointer', boxShadow:simStatus==='running'?'none':`0 0 14px ${C.red}60`, animation:simStatus==='running'?'urgentPulse 0.7s ease-in-out infinite':'none', transition:'box-shadow 0.2s, background 0.2s', whiteSpace:'nowrap' }}>
      ...
      <main style={{ padding:'20px 24px', maxWidth:1440, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18 }}>
          {['all','critical','active','blocked'].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding:'7px 18px', borderRadius:7, cursor:'pointer', fontSize:12, fontWeight:filter===f?700:400, textTransform:'uppercase', letterSpacing:'0.5px', transition:'all 0.2s', border:`1px solid ${filter===f?C.red:C.border}`, background:filter===f?`${C.red}15`:'transparent', color:filter===f?C.red:C.muted }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:18, marginBottom:18 }}>
          <div style={s.card}>
            ...
            <div style={{ overflowY:'auto', maxHeight:500 }}>
              {filtered.length===0 && <div style={{ padding:32, textAlign:'center', color:C.dim, fontSize:13 }}>No threats match this filter.</div>}
              {filtered.map(threat => (
                <div key={threat.id} onClick={() => setSelected(threat)} style={{ padding:'13px 20px', borderBottom:`1px solid ${C.border}`, borderLeft:`3px solid ${SEV[threat.severity]}`, cursor:'pointer', transition:'background 0.15s' }}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div style={{ ...s.card, padding:'16px 18px' }}>
              ...
              <div style={{ display:'flex', justifyContent:'center', marginBottom:4 }}><SecurityGauge score={Math.round(score)} /></div>
        ...
        <div style={{ ...s.card, padding:'20px 24px', marginBottom:18, position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <button onClick={runVulnScan} disabled={vulnScan==='scanning'} style={{ background:vulnScan==='scanning'?'transparent':`linear-gradient(135deg, ${C.blue}, #1d4ed8)`, color:vulnScan==='scanning'?C.blue:'#fff', border:`1.5px solid ${C.blue}`, borderRadius:7, padding:'8px 16px', fontSize:11, fontWeight:800, letterSpacing:'0.6px', textTransform:'uppercase', cursor:vulnScan==='scanning'?'not-allowed':'pointer', boxShadow:vulnScan==='scanning'?'none':`0 0 14px ${C.blue}50`, whiteSpace:'nowrap', animation:vulnScan==='scanning'?'scanPulse 0.8s ease-in-out infinite':'none', transition:'box-shadow 0.2s, background 0.2s' }}>{vulnScan==='scanning'?`🔍 SCANNING ${vulnProg}%`:'🔍 RUN VULNERABILITY SCAN'}</button>
            </div>
          </div>
        ...
        <div style={{ ...s.card, padding:'20px 24px', marginBottom:18 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:briefing?16:0 }}>
            ...
            <button onClick={generateBriefing} disabled={briefStatus==='loading'} style={{ background:briefStatus==='loading'?'transparent':`linear-gradient(135deg, #7c3aed, #6d28d9)`, color:briefStatus==='loading'?'#7c3aed':'#fff', border:'1.5px solid #7c3aed', borderRadius:7, padding:'8px 18px', fontSize:11, fontWeight:800, letterSpacing:'0.6px', textTransform:'uppercase', cursor:briefStatus==='loading'?'not-allowed':'pointer', boxShadow:briefStatus==='loading'?'none':'0 0 16px rgba(124,58,237,0.5)', whiteSpace:'nowrap', animation:briefStatus==='loading'?'scanPulse 0.8s ease-in-out infinite':'none', transition:'box-shadow 0.2s, background 0.2s' }}>{briefStatus==='loading'?'⚡ GENERATING...':'⚡ GENERATE AI BRIEFING'}</button>
          </div>
          {briefing && (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:14 }}>
            {[
              ...
              <div key={org} style={{ background:C.surface, borderRadius:8, padding:'16px 18px', borderLeft:`3px solid ${color}` }}>
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}><span style={{ fontSize:16 }}>{icon}</span><span style={{ fontWeight:800, fontSize:15, color }}>{org}</span></div>
